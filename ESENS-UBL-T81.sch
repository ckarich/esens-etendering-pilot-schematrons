...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" schemaVersion="iso" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <title>eSENS business and syntax rules for Expression of Interest</title>
    <title>Changelog
           * 2017-01-20:
             - eSENS-T81-R031: added rule for enforcing EO contact infos to be present
             - eSENS-T81-R032: added rule for enforcing Contactname and either one of Phone,Fax,Mail-Infos to be present  
           * 2017-01-13
             - eSENS-T81-R029: rewrote the test using xpath core functions to allow schematron validation in pure xpath contexts (mixing with non-xpath (xslt-only) functions lead to errors in some implementations), moved the rule to allready existing pattern/context
           * 2017-01-09:
             - eSENS-T81-R013: enforcing endpoint identifier for CB
             - eSENS-T81-R014: enforcing endpoint Identifier for EO
             - eSENS-T81-S002: rewrote the test to check for empty elements so its easier to understand/maintain
           * 2016-12-13:
             - eSENS-T81-S003: fixed text (was S300 instead of S003)
           * 2016-12-08:
             - eSENS-T81-S003 and all eSENS-T81-S3##: changed to warning to allow non-strict conformant instance documents, changed ruletexts from MUST to SHOULD
             - eSENS-T81-R031: deleted (enforcing UBLVersionID is not neccessary as discussed in the Leiden meeting)
           * 2016-11-18:
             - new rule eSENS-T81-R031: An Expression of Interest MUST have a syntax identifier
             - eSENS-T81-S302: Changed rule from 'UBLVersionID must not be used' to 'UBLVersionID must not contain any attributes' since R031 was introduced
             - eSENS-T81-R023: fixed ruletext (ruleid missmatch)
             - eSENS-T81-R029: fixed ruletext (ruleid missmatch)
             - eSENS-T81-R030: fixed ruletext (ruleid missmatch)
             - eSENS-T81-S322: fixed ruletext (ruleid missmatch)
           * 2016-11-14:
             - new rule eSENS-T81-R030: IssueTime MUST have a granularity of seconds
             - moved eSENS-T81-R011 and eSENS-T81-R030 to own rule-context for IssueTime 
             - eSENS-T81-S002: fixed test condition since it did not match pseudo-empty elements (textnodes with whitespace characters)
             - eSENS-T81-S318: ContractingParty/Party MAY now include EndpointID as well.
             - eSENS-T81-R012: fixed typo
             - various rules: fixed expression/typos in ruletexts
           * 2016-11-11:
             - renamed all eSENS-T81-S003.* rules to eSENS-T81-S3xx
           * 2016-11-07:
             - eSENS-T81-R021: Language Codes are not limited to EU Languages anymore; complete codelist for ISO639-1 included as a regexp including lowercase and uppercase codes
             - eSENS-T81-S002: fixed typo in rule ID (was: ESENS-T81-S002)
             - eSENS-T81-R028: value for Tenderer Role Code list (listID) fixed to 'TendererRole' (was: TENDERER_ROLE)
           * 2016-11-06: 
             - initial redraft
    </title>
    ...
    <ns prefix="ubl" uri="urn:X-test:UBL:Pre-award:ExpressionOfInterest"/>
    ...
    <pattern>
        ...
        <rule context="ubl:ExpressionOfInterest">
        ...
        <rule context="ubl:ExpressionOfInterest/cbc:CustomizationID">
        ...
        <rule context="ubl:ExpressionOfInterest/cbc:ProfileID">
        ...
        <rule context="ubl:ExpressionOfInterest/cbc:ID">
        ...
        <rule context="ubl:ExpressionOfInterest/cbc:ContractFolderID">
        ...
        <rule context="ubl:ExpressionOfInterest/cbc:IssueTime">
        ...
        <rule context="ubl:ExpressionOfInterest/cac:ContractingParty">
        ...
        <rule context="ubl:ExpressionOfInterest/cac:EconomicOperatorParty/cac:Party">
        ...
        <rule context="ubl:ExpressionOfInterest/cac:ContractingParty/cac:Party">
