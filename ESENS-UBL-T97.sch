...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <title>eSENS business and syntax rules for Tender Status Inquiry</title>
    <title>Changelog
        * 2017-01-17
          - eSENS-T97-S313: fixed the rule by asserting instead of reporting the check
        * 2017-01-13
          - eSENS-T97-R022: rewrote the test using xpath core functions to allow schematron validation in pure xpath contexts (mixing with non-xpath (xslt-only) functions lead to errors in some implementations), moved the rule to allready existing pattern/context
        * 2017-01-09:
          - eSENS-T97-R007: enforcing endpoint identifier for CB
          - eSENS-T97-R008: enforcing endpoint Identifier for EO
          - eSENS-T97-S002: rewrote the test to check for empty elements so it's easier to understand/maintain
        * 2016-12-13:
          - eSENS-T97-S003: fixed text (was S300 instead of S003)
        * 2016-12-08:
          - eSENS-T97-R023: deleted (enforcing UBLVersionID is not necessary as discussed in the Leiden meeting) // is now eSENS-T97-S302 again
          - eSENS-T97-S003 and all eSENS-T97-S3##: changed to warning to allow non-strict conformant instance documents, changed ruletexts from MUST to SHOULD
          - eSENS-T97-R010: changed customizationID temporarily to the value we have registered under in the SMP allthough the BIS as a customization of the profile/transaction should be used later
        * 2016-11-18:
          - new rule eSENS-T97-R023: A Tender Status Inquiry MUST have a syntax identifier
          - eSENS-T97-S302: Changed rule from 'UBLVersionID must not be used' to 'UBLVersionID must not contain any attributes' since R023 was introduced
          - eSENS-T97-R001: fixed typo in ruleID
        * 2016-11-16: 
          - initial draft
    </title>
    ...
    <ns prefix="ubl" uri="urn:X-test:UBL:Pre-award:TenderStatusInquiry"/>
    ...
    <pattern>
        ...
        <rule context="ubl:TenderStatusInquiry">
         ...
         <rule context="ubl:TenderStatusInquiry/cbc:CustomizationID">
            <assert id="eSENS-T97-R010" flag="fatal" test="normalize-space(.) = 'urn:www.cenbii.eu:transaction:biitrdm097:ver3.0'">[eSENS-T97-R010] CustomizationID value MUST be 'urn:www.cenbii.eu:transaction:biitrdm097:ver3.0'</assert>
        ...
        <rule context="ubl:TenderStatusInquiry/cbc:ProfileID">
        ...
        <rule context="ubl:TenderStatusInquiry/cbc:ID">
        ...
        <rule context="ubl:TenderStatusInquiry/cbc:ContractFolderID">
        ...
        <rule context="ubl:TenderStatusInquiry/cbc:IssueTime">
        ...
        <rule context="ubl:TenderStatusInquiry/cac:ContractingParty">
        ...
        <rule context="ubl:TenderStatusInquiry/cac:EconomicOperatorParty">
        ...
        <rule context="ubl:TenderStatusInquiry/cac:ContractingParty/cac:Party">
        ...
        <rule context="ubl:TenderStatusInquiry/cac:EconomicOperatorParty/cac:Party">
        ...
        <rule context="ubl:TenderStatusInquiry/cac:ProcurementProjectLot">
        ...
        <rule context="ubl:TenderStatusInquiry/cac:ProcurementProjectLot/cbc:ID">
